...
class Data:
    ...
    def _setDataframes(self):
        self.df_5m = pd.read_csv(base_path+'/data/'+self.coin+'_1m.csv',usecols=['Time', 'Open','High' ,'Low', 'Close','Volum'])#   
    ...
    def _dataProcess(self, msg):
        ...
        #self.df_4h = self.df_4h.drop_duplicates(subset=['Time'], keep='last')
        ...
        index_names = self.df_5m[ self.df_5m['Time'] == int(row['t']) ].index
        self.df_5m.drop(index_names, inplace = True)
        self.df_5m = self.df_5m.append(w, ignore_index=True )
    ...
    def _dataConvert(self, ms, df):
        ...
        if self.df_5m.iloc[-1].to_dict()['Time']%ms==0:
                 df = df.append(self.df_5m.tail(1), ignore_index=True)
            
        else:
            last_row =df.iloc[-1].to_dict('records')[0]
            row = self.df_5m.iloc[-1].to_dict('records')[0]
